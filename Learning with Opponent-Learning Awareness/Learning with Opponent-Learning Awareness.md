...
2. 假设条件：2个agent，agent最大化自己的累积收益，状态s是完全可观察的，在不同设置下分别是：设置1：可以完全访问对手的信息（expected total discounted return V，policy parameter $\theta$） 设置2：可以观察到对手的动作$u$
3. 主要想法：在做policy gradient update时，考虑对手对于你PG之后的反应，并利用这样的反应来修改PG的梯度
4. 解决方案：在设置1中，相比传统learner，引入一个包含对手Value function的Hessian的二阶项来来考虑对手的策略的变化。然后更进一步，比如不能访问Hessian的梯度的话，在设置1中使用PG来学习的话，将关于Value function变化为对手在轨迹$\tau$下的total discounted reward与策略。在设置2中，利用观察到的对手的action，对于对手的策略采用maximum likelihood建立opponent modeling，然后同设置1的做法来学习
...
假设agent1的policy $\pi^1$是由参数$\theta^1$表示，那么agent 1的expected total discounted return就可以写成$V^1(\theta^1, \theta^2)$，那么同样我们可以写出agent2的policy $\pi^2$，expected total discounted return $V^2(\theta^1, \theta^2)$

假设在时刻$t$时，agent1的策略为$\pi^1_t$, expected total discounted return为$V^1_t(\theta^1, \theta^2)$,agent2的策略为$\pi^2_t$, expected total discounted return为$V^2_t(\theta^1, \theta^2)$

那么在时刻$t$时，我们可以直观地利用当前的局面(即对手的策略)来选择下一个时刻$t+1$的策略，形式化地写成：
$$
\theta^1_{t+1} = argmax_{\theta^1} V^1(\theta^1, \theta_t^2)\\
\theta^2_{t+1} = argmax_{\theta^2} V^1(\theta_t^1, \theta^2)
$$
这里存在一个问题，当V的空间是无限大的时候（比如action是连续的），那么很多时候我们并不能直接通过求解上面的方程组来获得下一个时刻$t+1$的参数$\theta_{t+1}$。一种很直观的想法当然是通过求解梯度的方法来不断迭代：
$$
...
$$
其中 $\delta$是一阶的step size， $\eta$是二阶的step size

论文上没有从方程变化为迭代的方法的推导过程, 请教一下张程伟学长，我自己总结一下，因为我们的目的是：
$$
\Delta\theta^1 = \mathop{\arg\max}_{\Delta\theta^1:||\Delta\theta^1|| \leq\delta}V^1(\theta^1_t + \Delta\theta^1, \theta^2_t + \mathop{\arg\max}_{\Delta\theta^2:||\Delta\theta^2|| \leq\delta}V^2(\theta^1_t + \Delta\theta^1, \theta^2_t + \Delta\theta^2 ) )
$$
转化为梯度的方法的话，其实目的就是求解梯度的方向，从极限的角度来理解的话，以agent1为例，那么我们可以假设$\Delta\theta^1 \rightarrow 0$, 所以可以近似地看成对下面的式子求解梯度方向：
$$
...
那么它的梯度为：
$$
\frac{\partial V^1(\theta^1_t, \theta^2_t + \frac{\partial V^2(\theta^1_t, \theta^2_t)}{\partial \theta^2_t} \cdot \delta)}{\partial \theta^1_t} \approx \\
\frac{\partial V^1(\theta^1_t, \theta^2_t)}{\partial \theta^1_t} + \frac{\partial V^1(\theta^1_t, \theta^2_t)}{\partial \theta^2_t} \cdot \frac{\partial(\frac{\partial V^2(\theta^1_t, \theta^2_t)}{\partial \theta^2_t} \cdot \delta)}{\partial \theta^1_t} = 
\\
\frac{\partial V^1(\theta^1_t, \theta^2_t)}{\partial \theta^1_t} + (\frac{\partial V^1(\theta^1_t, \theta^2_t)}{\partial \theta^2_t})^T \cdot \frac{\partial^2 V^2(\theta^1_t, \theta^2_t))}{\partial \theta^1_t \partial \theta^2_t}  \cdot \delta
$$
那么最后乘上step size就近似为：
$$
f^1_{lola} = \frac{\partial V^1(\theta^1_t, \theta^2_t)}{\partial \theta^1_t} \cdot \delta + (\frac{\partial V^1(\theta^1_t, \theta^2_t)}{\partial \theta^2_t})^T\frac{\partial^2V^2(\theta^1_t, \theta^2_t)}{\partial \theta^1_{t} \partial\theta^2_t} \cdot \delta \eta
...
当不可以exact gradients of Hessians时候，也就是无法访问V的时候，我们可以采用在一条轨迹的$R_t^a(\tau) = \sum^T_{l=t}\gamma^{l-t}r_l^a$来替代$V^a$的作用，其中$\tau = (s_0, u_0^0, u_0^1, r_0^0, r_0^1, s_1...u_t^0, u_t^1 \, r_t^0, r_t^1)$

对于navie learner, 以agent1为例我们可以写成：
$$
f_{nl, pg}^{1} = \nabla_{\theta^1} \mathbb{E}(R^1_0(\tau)) \cdot \delta \\
其中：
\nabla_{\theta^1} \mathbb{E}(R^1_0(\tau)) = \mathbb{E}[R^1_0(\tau)\nabla_{\theta^1}log\pi^1(\tau)] \\
= \mathbb{E}[\sum^T_{t=0}\nabla_{\theta^1}log\pi^1(u^1_t|s_t)\gamma^t( R^1_0(\tau) - b(s_t))] 
$$

对于LOLA，我们可以写成：
$$
f_{lola, pg}^{1} = \nabla_{\theta^1} \mathbb{E}(R^1_0(\tau)) \cdot \delta + (\nabla_{\theta^2}\mathbb{E}R_0^1(\tau))^T\nabla_{\theta^1}\nabla_{\theta^2}\mathbb{E}R_0^2(\tau) \cdot \delta\eta \\
$$
...
在Learning via Policy Gradient中，虽然放宽了条件：不需要访问对手的V，但是需要访问对手的policy $\pi$。这个假设其实还是很强的，所以自然地，我们想要将这个强的限制放宽，但是算法本身是需要对手的信息（即对手的策略的），所以这里我们可以利用对手以前在状态$s$下的action $a$对对手建模，也就是设置2

将策略看成概率分布，那么就是一个估计的问题，自然会想到用maximum likelihood来估计：$\hat{\theta}  = \mathop{\arg\max}_{\theta}\sum_t log \pi_{\theta}(u_t|s_t)$
...
# Experimental Setup
## Iterated Games
可以简单地理解为：在每个状态$S_t$下都是同样的局面，能够采取的action一样，获得的reward一样。
...
对于上述的2个game，我们可以把它们model成two-agent MDP，状态$s_t = (u^1_{t-1}, u^2_{t-1})$，所以一共有5个state：s0, (C, C), (C, D), (D, C), (D, D)。
...
* batch size 4000.
* $\gamma$ is set to 0.96 for the prisoners’ dilemma and the coin game
* $\gamma$ is set to 0.9 for matching pennies
